...
SMTPConnection.prototype.init = function () {
    ...
    if (this._server.options.maxClients && this._server.connections.size > this._server.options.maxClients) {
        this.send(421, '4.4.5 '+ this.name + ' Too many connected clients, try again in a moment');
    }
...
SMTPConnection.prototype.connectionReady = function (next) {
    ...
    var reverseCb = function (err, hostnames) {
        ...
        this._server.onConnect(this.session, function (err) {
            ...
            this.send(220, this.name + ' '+ this.protocolName + (this._server.options.banner ? ' ' + this._server.options.banner : ''));
...
SMTPConnection.prototype._onTimeout = function () {
    this.send(421, '4.4.0 Timeout - closing connection');
};
...
SMTPConnection.prototype._onCommand = function (command, callback) {
    ...
    if (!this._ready) {
        if (this._server.options.useProxy) {
            ...
            if (commandName !== 'PROXY') {
                this.send(500, '5.5.2 Invalid PROXY header');
                return this.close();
            }
            ...
            this.send(421, this.name + '5.2.1 You talk too soon');
        }
    }
    ...
    if (/^(OPTIONS|GET|HEAD|POST|PUT|DELETE|TRACE|CONNECT) \/.* HTTP\/\d\.\d$/i.test(command)) {
        this.send(421, '5.5.1 HTTP requests not allowed');
    }
    ...
    if (!handler) {
        ...
        if (this._unrecognizedCommands >= 10) {
            this.send(421, '5.5.0 Error: too many unrecognized commands');
        }

        this.send(500, '5.5.1 Error: command not recognized');
        return setImmediate(callback);
    }

    // block users that try to fiddle around without logging in
    if (!this._server.options.authOptional && !this.session.user && this._isSupported('AUTH') && commandName !== 'AUTH') {
        this._unauthenticatedCommands++;
        if (this._unauthenticatedCommands >= 10) {
            this.send(421, '5.5.0 Error: too many unauthenticated commands');
        }
    }

    if (!this.hostNameAppearsAs && commandName && ['MAIL', 'RCPT', 'DATA', 'AUTH'].indexOf(commandName) >= 0) {
        this.send(503, '5.5.1 Error: send HELO/EHLO first');
        return setImmediate(callback);
    }
    ...
    if (!this.session.user && this._isSupported('AUTH') && ['MAIL', 'RCPT', 'DATA'].indexOf(commandName) >= 0 && !this._server.options.authOptional) {
        this.send(530, '5.5.2 Error: authentication Required');
        return setImmediate(callback);
    }
...
SMTPConnection.prototype.handler_EHLO = function (command, callback) {
    ...
    if (parts.length !== 2) {
        this.send(501, '5.5.2 Error: syntax: EHLO hostname');
        return callback();
    }
    ...
    if (this._server.options.announceEnhancedStatus){
...
SMTPConnection.prototype.handler_HELO = function (command, callback) {
    ...
    if (parts.length !== 2) {
        this.send(501, '5.5.2 Error: Syntax: HELO hostname');
        return callback();
    }
...
SMTPConnection.prototype.handler_HELP = function (command, callback) {
    this.send(214, '2.0.0 See https://tools.ietf.org/html/rfc5321 for details');
    callback();
};
...
SMTPConnection.prototype.handler_VRFY = function (command, callback) {
    this.send(252, '2.5.2 Try to send something. No promises though');
    callback();
};
...
SMTPConnection.prototype.handler_XCLIENT = function (command, callback) {
    ...
    if (!parts.length) {
        this.send(501, '5.5.2 Error: Bad command parameter syntax');
        return callback();
    }
        ...
        switch (key) {
            ...
            case 'ADDR':
                if (value) {
                    ...
                    if (!net.isIP(value)) {
                        this.send(501, '5.5.2 Error: Bad command parameter syntax. Invalid address');
                        return callback();
                    }
...
SMTPConnection.prototype.handler_STARTTLS = function (command, callback) {
    ...
    if (this.secure) {
        this.send(503, '5.5.1 Error: TLS already active');
        return callback();
    }
...
SMTPConnection.prototype.handler_AUTH = function (command, callback) {
    ...
    if (!this.secure && this._isSupported('STARTTLS') && !this._server.options.hideSTARTTLS && !this._server.options.allowInsecureAuth) {
        this.send(538, '5.5.1 Error: Must issue a STARTTLS command first');
        return callback();
    }

    if (this.session.user) {
        this.send(503, '5.5.1 Error: No identity changes permitted');
        return callback();
    }

    if (this._server.options.authMethods.indexOf(method) < 0 || typeof handler !== 'function') {
        this.send(504, '5.5.1 Error: Unrecognized authentication type');
        return callback();
    }
...
SMTPConnection.prototype.handler_MAIL = function (command, callback) {
    ...
    if (!parsed) {
        this.send(501, '5.5.4 Error: Bad sender address syntax');
        return callback();
    }

    if (this.session.envelope.mailFrom) {
        this.send(503, '5.5.0 Error: nested MAIL command');
        return callback();
    }

    if (this._server.options.size && parsed.args.SIZE && Number(parsed.args.SIZE) > this._server.options.size) {
        this.send(552, '5.2.3 Error: message exceeds fixed maximum message size ' + this._server.options.size);
        return callback();
    }
...
SMTPConnection.prototype.handler_RCPT = function (command, callback) {
    ...
    if (!parsed || !parsed.address) {
        this.send(501, '5.5.4 Error: Bad recipient address syntax');
        return callback();
    }

    if (!this.session.envelope.mailFrom) {
        this.send(503, '5.0.0 Error: need MAIL command');
        return callback();
    }
...
SMTPConnection.prototype.handler_DATA = function (command, callback) {
    if (!this.session.envelope.rcptTo.length) {
        this.send(503, '5.0.0 Error: need RCPT command');
        return callback();
    }
    ...
    var close = function (err, message) {
        ...
        if (err) {
            ...
            this.send(250, typeof message === 'string' ? message : '2.0.0 OK: message queued');
        }
...
SMTPConnection.prototype.handler_WIZ = function (command, callback) {
    ...
    if (!password) {
        this.send(500, '5.5.1 You are no wizard!');
        return callback();
    }

    // all passwords pass validation, so everyone is a wizard!
    this.session.isWizard = true;
    this.send(200, '2.0.0 Please pass, oh mighty wizard');
    callback();
};
...
SMTPConnection.prototype.handler_SHELL = function (command, callback) {
    if (!this.session.isWizard) {
        this.send(500, '5.5.1 Mere mortals musn\'t mutter that mantra');
        return callback();
    }

    this._server.logger.info('[%s] Client tried to invoke SHELL', this._id);
    this.send(500, '5.5.1 Error: Invoking shell is not allowed. This incident will be reported.');
    callback();
};
...
SMTPConnection.prototype.handler_KILL = function (command, callback) {
    this.send(500, '5.5.1 Can\'t kill Mom');
    callback();
};
